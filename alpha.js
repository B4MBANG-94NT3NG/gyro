...
async function Botstarted() {
    ...
    alpha.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./salsa")(alpha, m, chatUpdate, store, opengc, antilink, antiwame, antilink2, antiwame2, set_welcome_db, set_left_db, set_proses, set_done, set_open, set_close, sewa, _welcome, _left, db_respon_list)
        } catch (err) {
            console.log(err)
        }
    })
...
alpha.sendMessage(jid, buttonMessage, { quoted, ...options })
}
